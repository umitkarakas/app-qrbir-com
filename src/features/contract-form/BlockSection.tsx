...
export function BlockSection({ contract, entry, onUpdate, onAdd, onRemove }: Props) {
  const minItems = contract.minItems ?? (contract.required ? 1 : 0);
  const canAdd = !contract.maxItems || entry.items.length < contract.maxItems;
  const canRemove = entry.items.length > minItems;
